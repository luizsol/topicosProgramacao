...
/**
    ...
    @throw caso o arquivo não exista (invalid_argument)
*/
bool AcessoDados::atualizar(Arquivos arq, string valChave, Campos chave,
    ...
    while(!this->arquivo.eof() && getline(this->arquivo, linha)){
      if(linha != linhas[i]){
        ...
        case C_TIPOFUNC:
          ...
          break;
        default:
          throw std::invalid_argument("AcessoDados::atualizar: tentativa de "
                                      "conexao a um campo inexistente");
        }
  ...
  case CONTR_SINDICAL:
    ...
    break;
  case TAB_VALORHORA:
    nomeArq = nomesArqs[TAB_VALORHORA];
    break;
  case HORAS_AUTO:
    nomeArq = nomesArqs[HORAS_AUTO];
    break;
  default:
    throw std::invalid_argument("AcessoDados::atualizar: tentativa de "
...
bool AcessoDados::excluir(Arquivos arq, string valChave, Campos chave){
  ...
  case HORAS_AUTO:
    ...
    break;
  default:
    throw std::invalid_argument("AcessoDados::excluir: tentativa de "
                                "conexao a um arquivo inexistente");
  }
  AcessoDados::desconectar();
  arquivoAux.clear();
