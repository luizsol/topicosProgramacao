...
void Empresa::emitirFolhaPagamento(){
  ...
  vector<float> resultAcumAut({0, 0, 0, 0});
  vector<float> resultAcumMen({0, 0, 0, 0});

  // Autonomos

  cout << "Autonomos: " << endl << endl;
  for(unsigned long i = 0; i < this->funcionarios.size(); i++){
    if(this->funcionarios[i]->getTipoFuncionario() == "2"){
      vector<float> resultParcial = Empresa::getValoresSalariais(
        this->funcionarios[i]->getIdFuncional());
        resultAcumAut[0] += resultParcial[0];
        resultAcumAut[1] += resultParcial[1];
        resultAcumAut[2] += resultParcial[2];
        resultAcumAut[3] += resultParcial[3];
    }
  }

  stringstream salarioBrutoAut;
  salarioBrutoAut << "R$ " << fixed << setprecision(2) << resultAcumAut[0];

  stringstream descontoSindicalAut;
  descontoSindicalAut << "R$ " << fixed << setprecision(2) << resultAcumAut[1];

  stringstream impostoRendaAut;
  impostoRendaAut << "R$ " << fixed << setprecision(2) << resultAcumAut[2];

  stringstream salarioLiquidoAut;
  salarioLiquidoAut << "R$ " << fixed << setprecision(2) << resultAcumAut[3];

  cout << endl << "Parcial de salários brutos:\t" << salarioBrutoAut.str() << endl;
  cout << "Parcial de descontos sindicais:\t" << descontoSindicalAut.str() << endl;
  cout << "Parcial de impostos de renda:\t" << impostoRendaAut.str() << endl;
  cout << "Parcial de salários liquidos:\t" << salarioLiquidoAut.str() << endl;

  cout << endl << "Mensalistas: " << endl << endl;
  for(unsigned long i = 0; i < this->funcionarios.size(); i++){
    if(this->funcionarios[i]->getTipoFuncionario() == "1"){
      vector<float> resultParcial = Empresa::getValoresSalariais(
        this->funcionarios[i]->getIdFuncional());
        resultAcumMen[0] += resultParcial[0];
        resultAcumMen[1] += resultParcial[1];
        resultAcumMen[2] += resultParcial[2];
        resultAcumMen[3] += resultParcial[3];
    }
  }

  stringstream salarioBrutoMen;
  salarioBrutoMen << "R$ " << fixed << setprecision(2) << resultAcumMen[0];

  stringstream descontoSindicalMen;
  descontoSindicalMen << "R$ " << fixed << setprecision(2) << resultAcumMen[1];

  stringstream impostoRendaMen;
  impostoRendaMen << "R$ " << fixed << setprecision(2) << resultAcumMen[2];

  stringstream salarioLiquidoMen;
  salarioLiquidoMen << "R$ " << fixed << setprecision(2) << resultAcumMen[3];

  cout << endl << "Parcial de salários brutos:\t" << salarioBrutoMen.str() << endl;
  cout << "Parcial de descontos sindicais:\t" << descontoSindicalMen.str() << endl;
  cout << "Parcial de impostos de renda:\t" << impostoRendaMen.str() << endl;
  cout << "Parcial de salários liquidos:\t" << salarioLiquidoMen.str() << endl;

  // Total

  cout << endl << "Resultados totais da empresa " << Empresa::getNome() << endl <<
    endl;

  stringstream salarioBruto;
  salarioBruto << "R$ " << fixed << setprecision(2) << resultAcumMen[0] + resultAcumAut[0];

  stringstream descontoSindical;
  descontoSindical << "R$ " << fixed << setprecision(2) << resultAcumMen[1] + resultAcumAut[1];

  stringstream impostoRenda;
  impostoRenda << "R$ " << fixed << setprecision(2) << resultAcumMen[2] + resultAcumAut[2];

  stringstream salarioLiquido;
  salarioLiquido << "R$ " << fixed << setprecision(2) << resultAcumMen[3] + resultAcumAut[3];

  cout << endl << "Parcial de salários brutos:\t" << salarioBruto.str() << endl;
  cout << "Parcial de descontos sindicais:\t" << descontoSindical.str() << endl;
  cout << "Parcial de impostos de renda:\t" << impostoRenda.str() << endl;
  cout << "Parcial de salários liquidos:\t" << salarioLiquido.str() << endl;
...
vector<float> Empresa::getValoresSalariais(string idFuncional){
  ...
  if(this->funcionarios[index]->getTipoFuncionario() == "2"){
    ...
    cs = this->contribuicaoSindical.calcularCS(
      sb, this->funcionarios[index]->getFuncao());
  }
